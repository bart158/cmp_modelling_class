...
def Wavefunction(obj, *args, **kwargs):
    ...
    if (type(obj) == QuantumCircuit):
        statevec = S_simulator.run(transpile(obj, S_simulator)).result().get_statevector()
...
def Measurement(quantumcircuit, *args, **kwargs):
    ...
    if 'column' in kwargs:
        NL = kwargs['column']
    M1 = M_simulator.run(transpile(quantumcircuit, M_simulator), shots=S).result().get_counts(quantumcircuit)
    M2 = {}
    k1 = list(M1.keys())
    v1 = list(M1.values())
